                                    ...
                                    if (product.length > 0) {
                                        ...
                                        .reduce(function(promiesRes, productdata, index) {
                                            ...
                                            .catch(function(error) {
                                                ...
                                                res.send({
                                                    "status": 1,
                                                    "message": "product according to user",
                                                    "data": chkUser[0]
                                                });
                                            });
                                        } else {
                    ...
                    if (chat_result.length == 0) {
                        ...
                        cm.getAdminChat(function(err, admin_chat) {
                            if (admin_chat != 0) {
                                ...
                                admin_chat[0].userName = "KeyMarket Team";
...
app.post('/addProduct', function(req, res) {
    if (!req.body.user_pub_id || !req.body.cloudinary_id) {
        ...
        if (req.body.friend_pub_id) {
                ...
                var result = JSON.parse(JSON.stringify(subshistory))
                    ...
                    if(subshistory.length==0){
                        ...
                        if(intervalDate >= endTime){
                            ...
                            console.log('b',req.body.subscription_end_date)
                        }else{
                            
                            req.body.subscription_end_date = intervalDate;
                            console.log('c',req.body.subscription_end_date)
                        }
                    }
            ...
            delete req.body['language'];
            var intervals = req.body.interval;
            var intervalDate = moment().add(intervals, 'hours'); 
                                        ...
                                        cm.deleteProductReportbyUserPubId(req.body.user_pub_id, function(err, result) {
                                            //cm.deleteProductPurchasebyUserPubId(req.body.user_pub_id, function(err, result) {
                                                cm.deleteUserbyUserPubId(req.body.user_pub_id, function(err, result) {
                                                    cm.deleteViewbyUserPubId(req.body.user_pub_id, function(err, result) {
                                    ...
                                    cm.getallDataWhere('user', {
                                        ...
                                        if (chkUser.length > 0) {
                                            cm.getallDataWhere('followers', {
                                                ...
                                                cm.getallDataWhere('followers', {
                                                    user_pub_id: req.body.friend_pub_id,
                                                    following_user_pub_id: chkUser.pub_id
...
var Disc = require('./discrouter.js');
...
app.post("/Discoverdata",  function(req, res) {
    if(!req.body.user_pub_id || !req.body.latitude || !req.body.longitude || ! req.body.radius){
        ...
        var advertsql = "select * from advertisement where ad_status='1' and lang_id="+req.body.language;
        ...
        con.query(advertsql, function(err,advertisement_result){
            ...
            key.ad_banner = pakg_adver_url+key.ad_banner;
        ...
        cm.getCategoryWithLanguage(req.body.language, 'category', null, function(err,AllCat){
            ...
            .reduce(function(promiesRes, k, index) {
                ...
                .then(function(data) {
                    ...
                    return new Promise(function(resolve, reject) {
                        /*cm.getSubCategoryWithLanguage(req.body.language, k.category_id, function(sub_err, sub_cat_result){
                            //var allCat=JSON.parse(JSON.stringify(AllCat))
           
                            //console.log(AllCat);
                            
                        });*/
                        //console.log(k);
                        my.getSearchDataWithoutTag(req.body.latitude, req.body.longitude, 'product', {"category_pub_id":k.pub_id}, req.body.user_pub_id, req.body.radius, function(err, productData) {
                ...
                /*allCat.forEach(function (k, i, inner_callback){
                    var tot_c= (allCat.length);
